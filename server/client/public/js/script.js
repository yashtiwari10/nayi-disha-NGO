...
document.getElementById('donateSub').addEventListener('click', async () => {
  const btn = document.getElementById('donateSub');
  btn.disabled = true;
...
document.getElementById('volSub').addEventListener('click', async () => {
  const btn = document.getElementById('volSub');
  btn.disabled = true;
  ...
  if (!/^\d{10}$/.test(phone)) { setErr('verrPhone', 'Invalid phone'); valid = false; }
  if (!city) { setErr('verrCity', 'Enter city'); valid = false; }
  if (!skills.length) { setErr('verrSkill', 'Select skill'); valid = false; }
...
function showModal(name, amount, campaign) {
  ...
  document.getElementById('modalMsg').textContent =
    `₹${amount.toLocaleString('en-IN')} donated to "${campaign}". Thank you! 🙏`;
